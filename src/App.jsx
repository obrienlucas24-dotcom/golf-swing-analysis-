...
function BodyItem({ label, value }) {
  return (
    <div style={{ marginBottom: 14, paddingBottom: 14, borderBottom: `1px solid ${C.creamDark}` }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: C.gold, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>
        {label}
      </div>
...
function TipItem({ tip, index }) {
  return (
    <div style={{ display: 'flex', gap: 12, marginBottom: 16, paddingBottom: 16, borderBottom: `1px solid ${C.creamDark}` }}>
      <div style={{
        width: 28, height: 28, borderRadius: '50%', background: C.gold, flexShrink: 0,
...
function AnalyzeScreen({ history, setHistory }) {
  const [phase, setPhase] = useState('upload') // upload | preview | loading | results | error
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({ videoData: base64, mediaType: videoFile.type }),
      })
      if (!res.ok) {
    ...
    <div>
      ...
      <div style={{ padding: '20px 16px 24px' }}>
        ...
        {(phase === 'upload' || phase === 'error') && (
          <div className="fade-in-fast">
            ...
            <button
              ...
              style={{
                width: '100%', minHeight: 190, background: C.white,
                border: `2px dashed ${C.greenLight}`, borderRadius: 22,
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                justifyContent: 'center', gap: 14, cursor: 'pointer', marginBottom: 24,
              }}
            >
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 13, color: C.muted }}>Select a video from your camera roll</p>
              </div>
              <div style={{ background: C.green, color: C.cream, padding: '13px 32px', borderRadius: 50, fontSize: 14, fontWeight: 700 }}>
                Choose Video
              </div>
            </button>
            ...
            {history.length > 0 && (
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 12, margin: '0 0 12px' }}>Recent Swings</p>
        ...
        {phase === 'preview' && videoUrl && (
          <div className="fade-in-fast">
            <video
              ...
              style={{ width: '100%', borderRadius: 18, maxHeight: 300, background: '#000', marginBottom: 16, objectFit: 'contain', display: 'block' }}
            />
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={reset}
                style={{ flex: 1, padding: '15px 0', borderRadius: 50, border: `2px solid ${C.green}`, background: 'transparent', color: C.green, fontWeight: 600, fontSize: 15, cursor: 'pointer', minHeight: 52 }}
              >
                Change
              </button>
              <button
                onClick={handleAnalyze}
                style={{ flex: 2.2, padding: '15px 0', borderRadius: 50, background: C.green, color: C.cream, fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', minHeight: 52 }}
              >
                Analyze Swing ⛳
              </button>
            </div>
        ...
        {phase === 'loading' && (
          <div>
            {videoUrl && (
              <video src={videoUrl} playsInline muted style={{ width: '100%', borderRadius: 18, maxHeight: 220, background: '#000', marginBottom: 18, objectFit: 'contain', display: 'block' }} />
            )}
            <div style={{ textAlign: 'center', padding: '18px 0 26px' }}>
              ...
              <p style={{ fontWeight: 700, fontSize: 17, color: C.green, margin: 0 }}>Analyzing Your Swing</p>
              <p style={{ fontSize: 13, color: C.muted, marginTop: 5 }}>Your PGA coach is reviewing every frame…</p>
            </div>
        ...
        {phase === 'results' && analysis && (
          <div className="fade-in">
            {videoUrl && (
              <video src={videoUrl} controls playsInline style={{ width: '100%', borderRadius: 18, maxHeight: 220, background: '#000', marginBottom: 16, objectFit: 'contain', display: 'block' }} />
            )}
            ...
            <div style={{ background: C.green, borderRadius: 22, padding: '28px 24px 24px', marginBottom: 14, textAlign: 'center', boxShadow: '0 6px 24px rgba(26,58,42,0.28)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 18 }}>
                <ScoreRing score={analysis.score} animate />
              </div>
              {analysis.summary && (
                <p style={{ color: C.cream, fontSize: 13, lineHeight: 1.65, margin: 0, opacity: 0.88 }}>{analysis.summary}</p>
              )}
            </div>
            ...
            {analysis.bodyPosition && Object.keys(analysis.bodyPosition).length > 0 && (
              <Card title="Body Position Analysis" icon="🏌️">
                {Object.entries(analysis.bodyPosition).map(([key, val]) => (
                  <BodyItem key={key} label={key.replace(/([A-Z])/g, ' $1').trim()} value={val} />
                ))}
              </Card>
            )}
            ...
            <button
              onClick={reset}
              style={{ width: '100%', padding: '15px 0', borderRadius: 50, background: C.green, color: C.cream, fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', marginTop: 8, minHeight: 52 }}
            >
      ...
      <div>
        ...
        <div style={{ padding: '20px 16px 24px' }} className="fade-in">
          <div style={{ background: C.green, borderRadius: 22, padding: '26px 24px', marginBottom: 14, textAlign: 'center', boxShadow: '0 4px 20px rgba(26,58,42,0.22)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: selected.summary ? 16 : 0 }}>
          ...
          {selected.analysis?.phases?.length > 0 && (
            <Card title="Phase Breakdown" icon="📊">
              {selected.analysis.phases.map((p, i) => <PhaseRow key={i} phase={p} />)}
            </Card>
          )}
          {selected.analysis?.bodyPosition && (
            <Card title="Body Position" icon="🏌️">
              {Object.entries(selected.analysis.bodyPosition).map(([k, v]) => (
                <BodyItem key={k} label={k.replace(/([A-Z])/g, ' $1').trim()} value={v} />
              ))}
            </Card>
          )}
          {selected.analysis?.tips?.length > 0 && (
            <Card title="Coach's Tips" icon="💡">
              {selected.analysis.tips.map((t, i) => <TipItem key={i} tip={t} index={i} />)}
            </Card>
          )}
          <button
            onClick={() => {
              if (window.confirm('Delete this swing from history?')) {
                setHistory((prev) => prev.filter((h) => h.id !== selected.id))
                setSelected(null)
              }
            }}
            style={{ width: '100%', padding: '14px 0', borderRadius: 50, border: `2px solid ${C.errorBorder}`, background: 'transparent', color: C.error, fontWeight: 600, fontSize: 14, cursor: 'pointer', marginTop: 4 }}
          >
            Delete This Swing
          </button>
        </div>
  ...
  return (
    <div>
      <div style={{ background: C.green, padding: '54px 24px 28px' }}>
        ...
        <p style={{ color: C.cream, fontSize: 13, margin: '4px 0 0', opacity: 0.7 }}>
        ...
        {history.length === 0 ? (
          ...
          history.map((h) => (
            <button
              ...
              style={{
                width: '100%', background: C.white, border: 'none', borderRadius: 18,
                padding: '16px', display: 'flex', alignItems: 'center', gap: 14,
                marginBottom: 12, cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.07)',
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {h.summary && (
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.summary}</div>
                )}
              </div>
      ...
      <div style={{ background: C.green, padding: '54px 20px 0' }}>
        ...
        <div className="scroll-hide" style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 16 }}>
          {TIPS_DATA.map((cat, i) => (
            <button
              key={i}
              onClick={() => { setActiveCat(i); setExpanded(null) }}
              style={{
                flexShrink: 0, padding: '9px 18px', borderRadius: 50, border: 'none', cursor: 'pointer',
                background: activeCat === i ? C.gold : 'rgba(255,255,255,0.14)',
                color: activeCat === i ? C.green : C.cream,
      ...
      <div style={{ padding: '16px' }} className="fade-in-fast">
        {TIPS_DATA[activeCat].tips.map((tip, i) => (
          <button
            ...
            style={{
              width: '100%', background: C.white, border: 'none', borderRadius: 18,
              padding: '18px', textAlign: 'left', cursor: 'pointer', marginBottom: 12,
              boxShadow: '0 2px 12px rgba(0,0,0,0.07)', display: 'block', minHeight: 58,
            }}
          >
            ...
            {expanded === i && (
              <p style={{ margin: '12px 0 0', fontSize: 14, color: C.muted, lineHeight: 1.65 }}>{tip.body}</p>
            )}
          </button>
        ))}
      </div>
    ...
    <div>
      ...
      <div style={{ padding: '20px 16px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Sessions', value: history.length || '0' },
        ...
        {trend !== null && (
          <div style={{ background: trend >= 0 ? '#f0fdf4' : '#fef2f2', borderRadius: 14, padding: '14px 16px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: trend >= 0 ? '#15803d' : C.error, fontWeight: 600 }}>
              {trend >= 0 ? `+${trend}` : trend} points since your first session
            </p>
          </div>
        )}
        ...
        <Card title="About SwingAI" icon="ℹ️">
          <p style={{ fontSize: 13, color: C.muted, lineHeight: 1.65, margin: 0 }}>
            SwingAI uses advanced AI vision to analyze your golf swing frame by frame, providing expert-level feedback powered by Claude. Upload any swing video from your camera roll and receive a full breakdown in seconds.
          </p>
        </Card>

        <Card title="How It Works" icon="⚙️">
          {[
            ['📱', 'Upload video from your camera roll'],
            ['🤖', 'AI analyzes swing mechanics frame by frame'],
            ['📊', 'Receive a detailed phase-by-phase breakdown'],
            ['💡', 'Get personalized coaching tips to improve'],
          ].map(([icon, text]) => (
...
function BottomNav({ active, setActive }) {
  return (
    <div style={{
      ...
      background: C.green, borderTop: `1px solid ${C.greenLight}`,
      ...
      {NAV_ITEMS.map((n) => (
        <button
          ...
          style={{
            ...
            cursor: 'pointer', padding: '10px 0', minHeight: 60,
            borderTop: active === n.id ? `2px solid ${C.gold}` : '2px solid transparent',
            transition: 'border-color 0.15s',
          }}
        >
          <span style={{ fontSize: 21, lineHeight: 1 }}>{n.icon}</span>
          <span style={{ fontSize: 10, fontWeight: 700, color: active === n.id ? C.gold : 'rgba(245,240,232,0.45)', letterSpacing: '0.03em' }}>
            {n.label}
          </span>
        </button>
      ))}
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem('swingai_history', JSON.stringify(history))
  }, [history])

  return (
    <div style={{
      background: C.cream, minHeight: '100vh', maxWidth: 430,
